    ...
    for data_ in data:
        for paragraph in data_['paragraphs']:
            ...
            for qa in paragraph['qas']:
                q = '<p style="color:blue;">' + 'Q:  ' + qa['question'] + '</p>\n'
                html += q
...
def printTohtml_COQA(data):
    ...
    for data_ in data:
        ...
        for i in range(len(data_['questions'])):
            q = '<p style="color:blue;">' + 'Q:  ' + data_['questions'][i]['input_text'] + '</p>\n'
            html += q
...
def printTohtml_SMCalFlow(data):
    ...
    for dialog in data:
        for turn in dialog:
            user = '<p style="color:green;">' + 'user: ' + turn['user'] + '</p>\n'
            agent = '<p>' + 'agent: ' + turn['agent'] + '</p>\n'
            html += user
            html += agent
        html += '<p>' + '--------------------------------------------------------------------' + '</p>\n'
        # context
...
def printTohtml_QuAC(data):
    ...
    with open('QuAC.html', 'w', encoding='utf8') as f:
        f.write(html + "\n</body>\n</html>")
...
def if_match_quac(qa, sentence_end):
    if qa['answers'][0]['answer_start'] < sentence_end:
        return True
    else:
        return False
